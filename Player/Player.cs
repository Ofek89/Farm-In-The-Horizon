...
public class Player : SingletonMonobehaviour<Player>, ISaveable
{
    ...
    //watering_can coroutine timing
    private WaitForSeconds afterLiftToolAnimationPause;
    private WaitForSeconds liftToolAnimationPause;

    //hoe coroutine timing 
    private WaitForSeconds afterUseToolAnimationPause;
    private WaitForSeconds useToolAnimationPause;

    //collecting_tool coroutine timing
    ...
    [SerializeField] private SpriteRenderer equippedItemSpriteRenderer = null; //hold the sprite of the item the player is holding above the head
    ...
    private Direction playerDirection = default;
    ...
    protected override void Awake() {
        base.Awake(); //calles the base awake method defined in the singletonMonobehaviour class which we inherit from in this class
        rigidBody2D = GetComponent<Rigidbody2D>();
        mainCamera = Camera.main; //saved unity method to get a reference to the main camera
        animationOverrides = GetComponentInChildren<AnimationOverrides>();

        // Initialise swappable ARMS character attributes 
    ...
    private void Update() {
        ...
        if(!PlayerInputIsDisabled) {
            ResetAnimationTriggers(); //reset the animation triggers tha effect the animation
            PlayerMovementInput(); //check for player input and direction
            PlayerWalkInput(); //check if player is walking or running
            PlayerClickInput(); //check the condition to trigger depanding on item type the player clicked on the map with
           
            PlayerTestInput(); //TODO - remove when we no longer test player input function
    ...
    private void FixedUpdate() {
        PlayerMovement(); //moves the player using rb2d component
    }
    ...
    private void PlayerMovementInput()
    {
        ...
        if (yInput != 0 && xInput != 0) //if we press two buttons the same time
        {
            xInput = xInput * 0.71f;
            yInput = yInput * 0.71f;
        }

        if (xInput != 0 || yInput != 0) //player is moving
        {
            isRunning = true;
            isWalking = false;
            isIdle = false;
        ...
        else if (xInput == 0 && yInput == 0) //player isnt moving
        {
            isRunning = false;
            isWalking = false;
            isIdle = true;
    ...
    //player will run by default. if we press left/right shift key, the player will walk
    ...
    private void PlayerClickInput()
    {
        if (!playerToolUseDisabled)
        {
            if (Input.GetMouseButton(0)) //left mouse button click
            {
                ...
                {
                    // get grid and player positions to pass in as parameters for ProcessPlayerClickInput() method, to determine which side of animation to activate
    ...
    private void ProcessPlayerClickInput(Vector3Int cursorGridPosition, Vector3Int playerGridPosition) //Vector3Int cursorGridPosition, Vector3Int playerGridPosition
    {
        ResetMovement(); //resets players movement

        // Store player position for animation purposes, to run the currect side of animation depanding on both player and cursor position
        Vector3Int playerDirection = GetPlayerClickDirection(cursorGridPosition, playerGridPosition);

        // Get Grid prperty details at cursor position (the GridCursor validation routine ensures that grid property details are not null)
        GridPropertyDetails gridPropertyDetails = GridPropertiesManager.Instance.GetGridPropertyDetails(cursorGridPosition.x, cursorGridPosition.y);

        // Get Selected item details from the inventoryManager from the player inventory
        ItemDetails itemDetails = InventoryManager.Instance.GetSelectedInventoryItemDetails(InventoryLocation.player);

        if (itemDetails != null)
        {
            switch (itemDetails.itemType) //checks the type of the item
            {
                case ItemType.Seed:
    ...
    // Stores player position, so we know what is the currect animation to run, taking into account both player and cursor position
    ...
    // Player Click Input Methods used from within ProcessPlayerClickInput method
    #region Player Click Input Methods By ItemType
    private void ProcessPlayerClickInputSeed(GridPropertyDetails gridPropertyDetails, ItemDetails itemDetails)
    {
        //plant seed
        //if item can be dropped, cursor valid, ground has been dugged and hasnt been planted
        //(we need to paint the ground with the bool tilemap for canDropItem, so we can "drop plan" the seeds)
    ...
    //what happens when we click on a valid grid with the appropriate tool
    private void ProcessPlayerClickInputTool(GridPropertyDetails gridPropertyDetails, ItemDetails itemDetails, Vector3Int playerDirection)
    {
        ...
        switch (itemDetails.itemType)
        {
            ...
            case ItemType.Fishing_tool:
                if (gridCursor.CursorPositionIsValid)
                {
                    EventHandler.CallCastFishingEvent();
                }
                break;
    ...
    //ADDED
    //private void ProcessPlayerClickInputFishingTool(ItemDetails itemDetails)
    //{
    //    if (gridCursor.CursorPositionIsValid)
    //    {
    //        EventHandler.CallCastFishingEvent();
    ...
    private IEnumerator HoeGroundAtCursorRoutine(Vector3Int playerDirection, GridPropertyDetails gridPropertyDetails)
    {
        ...
        // pause this method while the animation is playing
        yield return useToolAnimationPause;

        // Set Grid property details for dug ground
        if (gridPropertyDetails.daysSinceDug == -1) //means it havnt been dugged
        {
    ...
    #endregion hoe methods ends

    #region Watering_can Methods
    ...
    #endregion Watering_can Methods ends

    #region Reaping_tool Methods
    ...
    #endregion Reaping_tool Methods

    #region Seed methods
    private void PlantSeedAtCursor(GridPropertyDetails gridPropertyDetails, ItemDetails itemDetails)
    {
        // Process if we have cropdetails for the seed in the SO_cropDetails list, to prevent an error
        ...
        {
            Debug.Log("Seed doesnt exist on the SO_cropDetails");
        }
    }
    #endregion Seed methods

    #region Collecting_tool Methods
    ...
    #endregion Collecting_tool Methods

    #region Chopping_tool Methods
    ...
    #endregion Chopping_tool Methods

    #region Breaking_tool Methods
    ...
    #endregion Breaking_tool Methods
    
    // Method processes crop with equipped item in player direction, used by chopping_tool and collecting_tool
    ...
    //used by the Reaping_tool IEnumerator method
            ...
            for (int i = itemArray.Length - 1; i >= 0; i--)
            {
                if (itemArray[i] != null)
                {
                    ...
                    if (InventoryManager.Instance.GetItemDetails(itemArray[i].ItemCode).itemType == ItemType.Reapable_scenary)
                    {
                        ...
                        // Trigger reaping effect, this also triggers a subscribed method from within the VFXManager script
    ...
    //determines what is the currect side to use the tool, according to the cursor and player positions
    ...
    private void PlayerTestInput()
    {
        ...
        // Trigger Object Pooling - Triggers an object pooling Prefab at mouse position
        //if (Input.GetMouseButtonDown(1))
        //{
        //    GameObject tree = PoolManager.Instance.ReuseObject(OakTreePrefabTest, mainCamera.ScreenToWorldPoint(new Vector3(Input.mousePosition.x,
    ...
    //used for moving the UI InventoryBar according to the player's position
    ...
    {
        return new Vector3(transform.position.x, transform.position.y + Settings.playerCentreYOffset,
